...
/**
 * 无人机实例参数（实例化机制：每个来源物品一个实例，各用各的物品级参数）
 * <p>
 * 机制集合 {@link #MECHANIC_SET}：装备中声明该集合的物品（或 Config 无人机模块物品）
 * 各自成为一个无人机实例，实例键为物品 ID。实例参数包括构建时间、基础生命值、
 * 基础伤害、基础数量、攻击间隔、移速倍率，未覆盖的参数回退 Config 默认值。
 */
public final class DroneInstanceParams {
    /** 无人机构建模块机制集合（实例来源物品声明此集合） */
    public static final String MECHANIC_SET = "drone_module_items";
    ...
    public static final String KEY_TARGET_RANGE = "target_range";

    /** 默认构建时间：与 DroneConstructTypes 注册的类型级构建时间一致 */
    private static final double DEFAULT_BUILD_TIME = 100.0D;

    private DroneInstanceParams() {}
    ...
    private static double fallbackFor(String paramKey) {
        return switch (paramKey) {
            ...
            case KEY_BASE_HEALTH -> Config.getDroneBaseHealth();
            case KEY_BASE_DAMAGE -> Config.getDroneBaseDamage();
            case KEY_BASE_COUNT -> Config.getDroneMaxCount();
            case KEY_ATTACK_INTERVAL -> 0.0D;
            case KEY_MOVE_SPEED -> 1.0D;
            case KEY_ATTACK_RANGE -> 0.0D;
            case KEY_TARGET_RANGE -> 0.0D;
            default -> 0.0D;
        };
    }
