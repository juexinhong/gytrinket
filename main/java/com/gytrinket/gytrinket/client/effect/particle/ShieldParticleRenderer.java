...
public class ShieldParticleRenderer {
    ...
    private static void renderPrism(BufferBuilder buf, Matrix4f matrix, ParticleGeo geo,
        ...
        if (renderFront) {
            for (int i = 0; i < n; i++) {
                int next = (i + 1) % n;
                buf.addVertex(matrix, fcx, fcy, fcz).setUv(outlineCenterU, outlineCenterV).setColor(frontColor);
                ...
                buf.addVertex(matrix, px0, py0, pz0).setUv(outlineUVs[i * 2], outlineUVs[i * 2 + 1]).setColor(frontColor);
                ...
                buf.addVertex(matrix, px1, py1, pz1).setUv(outlineUVs[next * 2], outlineUVs[next * 2 + 1]).setColor(frontColor);
            }
        }
        ...
        if (renderBack) {
            for (int i = 0; i < n; i++) {
                int next = (i + 1) % n;
                buf.addVertex(matrix, bcx, bcy, bcz).setUv(outlineCenterU, outlineCenterV).setColor(backColor);
                ...
                buf.addVertex(matrix, px1, py1, pz1).setUv(outlineUVs[next * 2], outlineUVs[next * 2 + 1]).setColor(backColor);
                ...
                buf.addVertex(matrix, px0, py0, pz0).setUv(outlineUVs[i * 2], outlineUVs[i * 2 + 1]).setColor(backColor);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            buf.addVertex(matrix, fpx0, fpy0, fpz0).setUv(u0, v0).setColor(sideColor);
            buf.addVertex(matrix, fpx1, fpy1, fpz1).setUv(u1, v1).setColor(sideColor);
            buf.addVertex(matrix, bpx0, bpy0, bpz0).setUv(u0, v0).setColor(sideColor);
            // 三角形 2: front[next], back[next], back[i]
            buf.addVertex(matrix, fpx1, fpy1, fpz1).setUv(u1, v1).setColor(sideColor);
            buf.addVertex(matrix, bpx1, bpy1, bpz1).setUv(u1, v1).setColor(sideColor);
            buf.addVertex(matrix, bpx0, bpy0, bpz0).setUv(u0, v0).setColor(sideColor);
        }
    }
