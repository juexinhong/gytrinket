...
public class EnergyWaveRenderManager {
    ...
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
        com.gytrinket.gytrinket.gytrinket.MODID, "textures/entity/swarm_attack.png"
    );
    ...
    private static void renderWaveTextured(Matrix4f matrix, BufferBuilder buffer, EnergyWaveData wave, float progress, float partialTick, Vec3 camPos) {
        ...
        int packedColor = ((int)(alpha * 255) << 24) | 0x00FFFFFF;
        ...
        // 水平平面（双面）
        buffer.addVertex(matrix, (float)hbl.x, (float)hbl.y, (float)hbl.z).setUv(0, 1).setColor(packedColor);
        buffer.addVertex(matrix, (float)hbr.x, (float)hbr.y, (float)hbr.z).setUv(1, 1).setColor(packedColor);
        buffer.addVertex(matrix, (float)htr.x, (float)htr.y, (float)htr.z).setUv(1, 0).setColor(packedColor);
        buffer.addVertex(matrix, (float)htl.x, (float)htl.y, (float)htl.z).setUv(0, 0).setColor(packedColor);
        ...
        // 垂直平面（双面）
        buffer.addVertex(matrix, (float)vbl.x, (float)vbl.y, (float)vbl.z).setUv(0, 1).setColor(packedColor);
        buffer.addVertex(matrix, (float)vbr.x, (float)vbr.y, (float)vbr.z).setUv(1, 1).setColor(packedColor);
        buffer.addVertex(matrix, (float)vtr.x, (float)vtr.y, (float)vtr.z).setUv(1, 0).setColor(packedColor);
        buffer.addVertex(matrix, (float)vtl.x, (float)vtl.y, (float)vtl.z).setUv(0, 0).setColor(packedColor);
    ...
    private static void renderWaveWhiteOverlay(Matrix4f matrix, BufferBuilder buffer, EnergyWaveData wave, float progress, float partialTick, Vec3 camPos) {
        // 白色覆盖透明度：开始为1（完全覆盖），随progress降低到0
        float overlayAlpha = (1.0f - progress) * (1.0f - progress);
        float scale = SIZE_SCALE * (1.0f + (END_SCALE - 1.0f) * progress);
        float forwardOffset = FORWARD_DRIFT * SIZE_SCALE * progress;

        // 白色顶点色 + overlayAlpha：材质alpha会自然过滤透明区域
        int packedColor = ((int)(overlayAlpha * 255) << 24) | 0x00FFFFFF;
