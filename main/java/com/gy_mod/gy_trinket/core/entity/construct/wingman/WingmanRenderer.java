 ...
 */
public class WingmanRenderer extends LivingEntityRenderer<WingmanConstructEntity, WingmanEntityModel<WingmanConstructEntity>> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("gytrinket", "textures/entity/wingman.png");
    private static final ResourceLocation GLOW_TEXTURE = new ResourceLocation("gytrinket", "textures/entity/wingman2.png");
    ...
    public WingmanRenderer(EntityRendererProvider.Context context) {
        ...
        // 发光层
        this.addLayer(new RenderLayer<WingmanConstructEntity, WingmanEntityModel<WingmanConstructEntity>>(this) {
            @Override
            public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, WingmanConstructEntity entity,
                ...
                getParentModel().renderToBuffer(poseStack, glowBuffer, packedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            }
        });
    }
    ...
    protected void scale(WingmanConstructEntity entity, PoseStack poseStack, float partialTickTime) {
        // 1.20.1 LivingEntityRenderer 不会自动应用 getScale，此处手动应用大小属性倍率
        float sizeMultiplier = (float) entity.getSizeMultiplier();
        poseStack.scale(sizeMultiplier, sizeMultiplier, sizeMultiplier);
    }

    @Override
    protected void setupRotations(WingmanConstructEntity entity, PoseStack poseStack, float ageInTicks, float rotationYaw, float partialTick) {
        // 偏航：标准方向
    ...
    @Override
    protected void renderNameTag(WingmanConstructEntity entity, Component displayName, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        // 不渲染僚机上方"僚机"的名牌
    }
