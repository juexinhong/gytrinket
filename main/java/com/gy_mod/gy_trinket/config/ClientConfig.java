...
public class ClientConfig {
    ...
    // ===== 护盾粒子渲染 =====
    public static final ForgeConfigSpec.ConfigValue<Boolean> SHIELD_PARTICLE_VOLUMETRIC_RENDERING;
    ...
    static {
        ...
        BUILDER.comment("护盾粒子渲染配置").push("shield_particle_rendering");

        SHIELD_PARTICLE_VOLUMETRIC_RENDERING = BUILDER.comment(
            "是否启用护盾粒子体积渲染（宝石质感）",
            "启用：使用自定义着色器渲染3D棱柱体，具有宝石/水晶质感",
            "禁用：使用原版透明度渲染（平面四边形）",
            "默认启用"
        ).define("volumetricRendering", true);
        ...
        HUD_VANILLA_TEXT_OFFSET_X = BUILDER.comment(
            ...
            "默认0"
        ).defineInRange("textOffsetX", -93, -500, 500);

        HUD_VANILLA_TEXT_OFFSET_Y = BUILDER.comment(
            "原版样式护盾值文本的Y偏移量",
            "基于纹理上方，0为默认位置，正数向下，负数向上",
            "默认0"
        ).defineInRange("textOffsetY", 12, -500, 500);
