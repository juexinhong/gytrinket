...
public class BowWeaponMode implements InterceptorWeaponMode {
    ...
    public void executeAttack(WingmanConstructEntity attacker, LivingEntity target, ItemStack weapon, Player owner) {
        ...
        AbstractArrow arrow = ProjectileUtil.getMobArrow(attacker, ammoStack, chargeFactor, weapon);

        // 弓自定义箭矢（支持药箭等特殊箭矢）
        if (weapon.getItem() instanceof BowItem bowItem) {
            arrow = bowItem.customArrow(arrow, ammoStack, weapon);
        }
    ...
    /**
     * 获取附魔等级（与女仆模组一致，通过 RegistryAccess 获取 Holder）
     */
    private int getEnchantmentLevel(WingmanConstructEntity attacker, ItemStack stack,
        ...
        var access = attacker.level().registryAccess();
        var holder = access.registryOrThrow(Registries.ENCHANTMENT).getHolderOrThrow(enchantmentKey);
