        ...
        for (Map.Entry<String, List<ItemStack>> entry : collectedMaterials.entrySet()) {
            ...
            for (ItemStack stack : entry.getValue()) {
                if (!stack.isEmpty()) {
                    ...
                    stack.save(net.neoforged.neoforge.server.ServerLifecycleHooks.getCurrentServer().registryAccess(), itemTag);
                    listTag.add(itemTag);
                }
            }
            if (!listTag.isEmpty()) {
    ...
    public static UpgradeData load(CompoundTag tag) {
        ...
        for (String key : tag.getAllKeys()) {
            ...
            for (int i = 0; i < listTag.size(); i++) {
                ...
                ItemStack stack = ItemStack.parseOptional(net.neoforged.neoforge.server.ServerLifecycleHooks.getCurrentServer().registryAccess(), itemTag);
                if (!stack.isEmpty()) {
                    materials.add(stack);
                }
            }
