...
public class InterceptorConfigContainer extends AbstractContainerMenu {
    ...
    public static final MenuType<InterceptorConfigContainer> TYPE = IMenuTypeExtension.create(
            (windowId, inv, data) -> {
                ItemStack weapon = ItemStack.OPTIONAL_STREAM_CODEC.decode(data);
                ItemStack ammo = ItemStack.OPTIONAL_STREAM_CODEC.decode(data);
    ...
    // 客户端构造器（从 RegistryFriendlyByteBuf 读取）
    public InterceptorConfigContainer(int containerId, Inventory inventory, RegistryFriendlyByteBuf data) {
        this(containerId, inventory, ItemStack.OPTIONAL_STREAM_CODEC.decode(data), ItemStack.OPTIONAL_STREAM_CODEC.decode(data), data.readUtf());
    }
    ...
    public InterceptorConfigContainer(int containerId, Inventory inventory, ItemStack weapon, ItemStack ammo, String attackModeName) {
        ...
        // 玩家背包（3行9列，间隔 20px，格子 18px → 2px 间隙）
    ...
    public void removed(Player player) {
        ...
        if (!player.level().isClientSide) {
            ...
            InterceptorWeaponManager.setWeapon(playerUUID, interceptorSlots.getItem(0));
            InterceptorWeaponManager.setAmmo(playerUUID, interceptorSlots.getItem(1));
            // 刷新所有僚机实体 + 持久化到 PlayerDataAttachment
            if (player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
                InterceptorWeaponManager.refreshAllWingmen(serverPlayer);
                InterceptorWeaponManager.saveToAttachment(serverPlayer);
            }
        }
    }
}
