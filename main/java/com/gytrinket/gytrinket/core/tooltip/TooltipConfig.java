...
public class TooltipConfig {
    ...
    /**
     * tooltip 格式化参数 source（Config 名）到机制数值参数（机制集合名 -> 参数键）的映射。
     * 命中映射的参数在渲染 tooltip 时优先取当前物品定义的覆盖值（未覆盖回退 Config 默认值）。
     */
    ...
    /**
     * 检查指定物品ID是否匹配此配置（从客户端同步的 item_sets registry 查询）
     */
    public boolean matchesItem(String itemId) {
        net.minecraft.client.Minecraft mc = net.minecraft.client.Minecraft.getInstance();
        net.minecraft.core.RegistryAccess access = mc.level != null ? mc.level.registryAccess() : null;
        return DefsManager.itemSetContains(access, itemSetName, itemId);
    }

    /**
     * 解析数据驱动的格式化参数
     * 类型见 {@link DefsManager.TooltipParam} 注释
     * 数值类参数优先取当前物品定义的机制数值覆盖（未覆盖回退 Config 默认值）
     */
    ...
    /**
     * 参数原始数值：物品覆盖值优先，未覆盖时回退 Config 配置值
     */
    ...
    /**
     * 机制参数取值（tooltip 显示"当前定义的值"）：若映射表命中且当前物品定义了覆盖值，
     * 返回覆盖值；否则返回传入的默认值。
     * 供数据驱动参数解析与硬编码模块 tooltip 共用。
     */
